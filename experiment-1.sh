...
(time python ./Workflow/middleExpand.py report all) 2>&1 | tee "/results/spmm-report-${SIMD_WIDTH}.log"
...
do
    ...
    (time python ./Workflow/middleExpand.py report ProposedSIMD) 2>&1 | tee "/results/spmm-report-${SIMD_WIDTH}.log"
done

mkdir /results/spmm
cp -r csvResults/ /results/spmm/
cp -r workload/ /results/spmm/
